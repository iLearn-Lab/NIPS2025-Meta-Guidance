...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            self.dec_embedding = DataEmbedding(configs.dec_in, 2*configs.d_model, configs.embed, configs.freq,
                                               configs.dropout)
            self.decoder = Decoder(
                [
                    DecoderLayer(
                        AttentionLayer(
                            ...
                            2*configs.d_model, configs.n_heads),
                        AttentionLayer(
                            DSAttention(False, configs.factor, attention_dropout=configs.dropout,
                                        output_attention=False),
                            2*configs.d_model, configs.n_heads),
                ...
                norm_layer=torch.nn.LayerNorm(2*configs.d_model),
                projection=nn.Linear(2*configs.d_model, configs.c_out, bias=True)
            )
        ...
        if self.task_name == 'classification':
            ...
            self.projection = nn.Linear(2*configs.d_model * configs.seq_len, configs.num_class)
    ...
    def imputation(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask):
        ...
        # get tau delta
        mean_enc = torch.sum(x_raw, dim=1) / torch.sum(mask == 1, dim=1)
        mean_enc = mean_enc.unsqueeze(1).detach()
        x_norm = x_raw - mean_enc
        x_norm = x_norm.masked_fill(mask == 0, 0)
        std_enc = torch.sqrt(torch.sum(x_norm * x_norm, dim=1) / torch.sum(mask == 1, dim=1) + 1e-5)
        ...
        # RevIN_weight
        dec_out = self.rev_weight(dec_out, 'denorm', None, None)
        return dec_out
